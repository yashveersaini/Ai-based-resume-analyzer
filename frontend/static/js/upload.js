...
function displaySuggestions(data) {
  ...
  jobCards.innerHTML = suggestions.map((job, i) => `
    <div class="job-card" onclick="openModal(${i})">
      ...
      <div class="job-info">
        ...
        <span class="job-confidence" style="margin-left: 8px;">
          ${job.matched_skills.length}/${job.total_skills} skills
        </span>
      </div>
    </div>
  `).join('');
...
function openModal(index) {
  ...
  document.getElementById('modalRankBadge').textContent = `Rank #${index + 1}`;
  document.getElementById('modalTitle').textContent = job.role;
  document.getElementById('modalConfidence').textContent = 
    `${job.confidence}% confidence · ${job.matched_skills.length}/${job.total_skills} skills matched`;
  
  // Matched skills
  const skillsEl = document.getElementById('modalMatchedSkills');
  if (job.matched_skills.length === 0) {
    skillsEl.innerHTML = '<div class="empty-state">None of your skills match this role</div>';
  } else {
  ...
  roadmapEl.innerHTML = job.roadmap.map(step => `
